...
export default function ListBox({ options, callBack, text }) {
                        ...
                        <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            {options.map((option, index) => (
                                <Listbox.Option
                                    key={index}
                                    className={({ active }) =>
                                        `rounded-md ${active ? 'bg-indigo-700 text-white font-semibold text-amber-900 bg-amber-100' : 'text-gray-900'}
                                ...
                                >
                                    {({ selected, active }) => (
                                        <>
                                            <span
                                                className={`${selected || active ? 'font-medium' : 'font-normal'
                                                    } block truncate`}
                                            >
                                                {option}
                                            </span>
